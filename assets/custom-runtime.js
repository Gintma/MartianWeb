...
function initMenuPopup() {
  ...
  const topMask = document.querySelector(".menu-popup__img-top-mask");
  const bottomMask = document.querySelector(".menu-popup__img-bottom-mask");
  const topLine = document.querySelector(".menu-popup__img-top-line");
  const bottomLine = document.querySelector(".menu-popup__img-bottom-line");
  const topLineMob = document.querySelector(".menu-popup__img-top-line-mob");
  const bottomLineMob = document.querySelector(".menu-popup__img-bottom-line-mob");
  const bottomItems = document.querySelector(".container-bottom__p-items");
  const popupItems = Array.from(
    menuPopup ? menuPopup.querySelectorAll(".menu-popup__item-p") : [],
  );
  const popupCloseBlock = document.querySelector(".menu-popup__close-block");
  ...
  document.querySelectorAll(".black-div-b, .fixed-black-line-l, .fixed-black-line-r").forEach((el) => {
  ...
  function setMenuItemWidth(item, value) {
    if (!item) return;
    item.style.overflow = "hidden";
    item.style.width = value;
  }

  function getNaturalItemWidth(item) {
    if (!item) return "0px";
  ...
  function applyBurgerState(open, desktop) {
    if (burgerLine1) {
  ...
  hoverablePopupItems.forEach((item) => {
    const label = item.querySelector(".p-big-caps");
    if (!label) return;
